...
function tampilbuku()
{
    ...
    if(!empty($result)){
        foreach ($result as $buku) {
            ...
            echo "<a href='Buku.php?id_buku=" . $buku['id_buku'] . "' onclick=\"return confirm
            ('Hapus Buku ?')\">Hapus Buku </a> </td>";
            echo "</tr>";
        }
    }
}
...
function tambahBuku($id,$judul,$penulis,$penerbit,$tahun)
{
    ...
    (:id_b,:judul_b,:penulis,:penerbit,:tahun)";
    $stmt = koneksi()->prepare($sql);
    $result = $stmt->execute(array(':id_b' => $id, 'judul_b'=>$judul,':penulis'=>
    $penulis,':penerbit'=>$penerbit,':tahun'=>$tahun));
    if (!empty($result)) {
...
function updatebuku($id,$judul,$penulis,$penerbit,$tahun)
{
    $pdo_statement = koneksi()->prepare(
        "update buku set judul_buku='" . $judul . "', penulis='" . $penulis . "', 
...
function tampilmember()
{
    ...
    if(!empty($result)){
        foreach ($result as $member) {
            ...
            ."'>Edit Member </a>";
            echo "<a href='Member.php?id_member=" . $member['id_member'] . "' 
            onclick=\"return confirm('Delete member?')\">Hapus Member </a> </td>";
            echo "</tr>";
        }
    }
}
...
{
    $sql = "INSERT INTO member(id_member,nama_member,nomor_member,alamat,tgl_mendaftar
    ,tgl_terakhir_bayar)values(:id_m,:nama_m,:no_m,:alamat,:tgl_daf,:tgl_bay)";
    $stmt = koneksi()->prepare($sql);
    $result = $stmt->execute(array(':id_m' => $id, 'nama_m'=>$nama,':no_m'=>$nomor,
    ':alamat'=>$alamat,':tgl_daf'=>$tgldaftar,':tgl_bay'=>$tglbayar));
    if (!empty($result)) {
...
{
    $pdo_statement = koneksi()->prepare(
        "update member set nama_member='" . $nama . "', nomor_member ='" . $no .
         "', alamat='" . $alamat . "', tgl_mendaftar='" . $tgldaftar . "',
          tgl_terakhir_bayar='".$tglbayar."' where id_member=" . $id
    );
    $result = $pdo_statement->execute();
    if($result){
        header('location:Member.php');
...
function tampilpeminjaman()
{
    ...
    if(!empty($result)){
        foreach ($result as $peminjaman) {
            ...
            echo "<td>"."<a href = 'formpeminjaman.php?id_peminjaman=".$peminjaman
            ['id_peminjaman']."'>Edit Data Peminjaman       </a>";
            echo "<a href='Peminjaman.php?id_peminjaman=" . $peminjaman['id_peminjaman']
             . "' onclick=\"return confirm('Hapus data?')\">Hapus Data peminjaman </a> </td>";
            echo "</tr>";
        }
    }
}
...
function tambahpeminjaman($id,$tglpinjam,$tglbalik)
{
    ...
    (:id_p,:tgl_pinjam,:tgl_balik)";
    $stmt = koneksi()->prepare($sql);
    $result = $stmt->execute(array(':id_p' => $id,':tgl_pinjam'=>$tglpinjam,
    ':tgl_balik'=>$tglbalik));
    if (!empty($result)) {
...
{
    $pdo_statement = koneksi()->prepare(
        "update peminjaman set tgl_pinjam='" . $tglpinjam . "', tgl_kembali='" 
        . $tglbalik . "' where id_peminjaman=" . $id
    );
    $result = $pdo_statement->execute();
    if($result){
        header('location:Peminjaman.php');
