...
function tampilbuku()
{
    ...
    if(!empty($result)){
        foreach ($result as $buku) {
            ...
            echo "<a href='Buku.php?id_buku=" . $buku['id_buku'] . "' onclick=\"return confirm('Hapus Buku ?')\">Hapus Buku </a> </td>";
            echo "</tr>";
        }
    }
}
...
function tampilmember()
{
    ...
    if(!empty($result)){
        foreach ($result as $member) {
            ...
            echo "<a href='Member.php?id_member=" . $member['id_member'] . "' onclick=\"return confirm('Delete member?')\">Hapus Member </a> </td>";
            echo "</tr>";
        }
    }
}
...
function tampilpeminjaman()
{
    ...
    if(!empty($result)){
        foreach ($result as $peminjaman) {
            ...
            echo "<a href='Peminjaman.php?id_peminjaman=" . $peminjaman['id_peminjaman'] . "' onclick=\"return confirm('Hapus data?')\">Hapus Data peminjaman </a> </td>";
            echo "</tr>";
        }
    }
}
...
{
    $pdo_statement = koneksi()->prepare(
        "update peminjaman set tgl_pinjam='" . $tglpinjam . "', tgl_kembali='" . $tglbalik . "' where id_peminjaman=" . $id
    );
    $result = $pdo_statement->execute();
    if($result){
        header('location:Peminjaman.php');
